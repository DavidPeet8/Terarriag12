...
public class ScrPlay implements Screen, InputProcessor {
    ...
    SpriteHero sprPlayer = new SpriteHero(texPlay, nPlayerSpawnX, nPlayerSpawnY, 50, 48, 64,
            150, 100, 150, 100);
    
    
    ArrayList<Enemy> alEnemies = new ArrayList<Enemy>();
    ...
    public ScrPlay(GamTerarria game) {
        ...
        for (int i = alEnemies.size() - 1; i > 0; i --) {
            alEnemies.get(i).init();
        }
...
//-------------Placing--------------- MUST BE LAST HAS POSSIBILITY TO MAKE NULL
                ...
                { //not inside yourself
                    ...
                    try {
                        if (ScrLoad.artBoxes[nMouseYtile][nMouseXtile] == null) {
                            ...
                            objInventory.getActive().alterStack(-1);
                        }
                    } catch (Exception e) {
                        System.out.println(e);
                    }
    ...
    public void createSubsetEnemy(Enemy en){
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                try{
                    nXSub = (int) en.getX() / Constants.TILEWIDTH - 4 + nOffsetX + x;
                    nYSub = (int) en.getY() / Constants.TILEHEIGHT - 4 + nOffsetY + y;
                    artSubsetBoxes[y][x] = ScrLoad.artBoxes[nYSub][nXSub];
                }catch (Exception e){}
            }
        }
    ...
    public void setPlayer(SpriteHero updatedPlayer){
    ...
    public void render(float delta) {
        ...
        for (int i = alEnemies.size() - 1; i > 0; i --) {
            Enemy e = alEnemies.get(i);
            createSubsetEnemy(e);
            e.update(artSubsetBoxes);
        }
